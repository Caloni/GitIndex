...
import subprocess
from subprocess import check_output as qx, Popen, PIPE
import argparse
...
def indexpdb(pdbpath):
  ...
  print >>pdbStreamFile, 'VERCTRL=Bazaar'
  print >>pdbStreamFile, 'DATETIME=Sun May 24 22:51:33 2015'
  print >>pdbStreamFile, 'SRCSRV: variables ------------------------------------------'
  print >>pdbStreamFile, 'BZR_REVNO=' + revno
  print >>pdbStreamFile, 'BZR_WEB_ADDRESS=' + args.webAddr
  print >>pdbStreamFile, 'BZR_URL=%bzr_web_address%/%bzr_revno%/%var2%'
  print >>pdbStreamFile, 'BZR_EXTRACT_TARGET=%targ%\%fnfile%(%var1%)'
  print >>pdbStreamFile, 'BZR_EXTRACT_CMD=cmd /c curl.exe "%bzr_url%" > "%bzr_extract_target%"'
  print >>pdbStreamFile, 'SRCSRVTRG=%bzr_extract_target%'
  print >>pdbStreamFile, 'SRCSRVCMD=%bzr_extract_cmd%'
